                                ...
                                <div class="form-group">
                                    ...
                                    <select name="platform_id" id="platform_id" class="form-control @error('platform_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($platforms as $platform)
                                            <option value="{{ $platform->id }}" {{ old('platform_id', $region->platform_id) == $platform->id ? 'selected' : '' }}>
                                                {{ $platform->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="region_code" id="region_code" class="form-control @error('region_code') is-invalid @enderror" 
                                           value="{{ old('region_code', $region->region_code) }}" placeholder="例如：ap-beijing-1" required>
                                    @error('region_code')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="region_name" id="region_name" class="form-control @error('region_name') is-invalid @enderror" 
                                           value="{{ old('region_name', $region->region_name) }}" placeholder="例如：北京一区" required>
                                    @error('region_name')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
