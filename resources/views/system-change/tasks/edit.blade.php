...
$(document).ready(function() {
    // 初始化已选择的服务器
    @if($task->server_group_id)
        loadServers({{ $task->server_group_id }}, {{ json_encode($task->server_ids ?? []) }});
    @endif
...
function loadServers(groupId, selectedIds = []) {
    $.get('{{ route("system-change.server-groups.get-servers") }}', {
...
function loadVariables() {
    ...
    $.post('{{ route("system-change.templates.get-variables") }}', {
        _token: '{{ csrf_token() }}',
        template_ids: selectedTemplates
    }).done(function(data) {
        if (data.variables && Object.keys(data.variables).length > 0) {
            var html = '';
            var existingValues = @json(old('config_variables', $task->config_variables ?? []));
